...
rdict = requests.get('https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/perks.json').json()
runeDict = {}
for i in rdict:
    runeDict[i['name']] = i['id']

rhead = requests.get('http://ddragon.leagueoflegends.com/cdn/11.11.1/data/en_US/runesReforged.json').json()
runeHeading = {}
for i in range(5):
...
def uggParsing(champion):
    ...
    y = soup.find('div', class_='rune-trees-container-2 media-query media-query_MOBILE_LARGE__DESKTOP_LARGE')
    ...
    sub_tree = soup.find('div',
                             class_='rune-trees-container-2 media-query media-query_MOBILE_LARGE__DESKTOP_LARGE').find(
...
async def get_details(connection, champion):
    ...
    Perks = uggParsing(champion)
    await connection.request('delete', '/lol-perks/v1/pages' + '/' + str(currentPageId))
    newRune = {"primaryStyleId": Perks[0], "subStyleId": Perks[1],
               "selectedPerkIds": Perks[2:11],
               "name":champion}
    await connection.request('post','/lol-perks/v1/pages',data=newRune)
    print("RUNES HAVE BEEN UPDATED")


@connector.ws.register('/lol-gameflow/v1/session', event_types =('UPDATE', 'CREATE', 'DELETE', ))
...
@connector.ws.register('/lol-champ-select/v1/current-champion', event_types=('UPDATE', 'CREATE', 'DELETE',))
